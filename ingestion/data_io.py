...
def read_as_df(basename, type="train"):
    ...
    feat_name = read_csv(basename + '_feat.name', header=None)
    label_name = read_csv(basename + '_label.name', header=None)
    X = read_csv(basename + '_' + type + '.data', sep=' ', names = np.ravel(feat_name))
    ...
    if isfile(solution_file):
        Y = read_csv(solution_file, sep=' ', names = np.ravel(label_name))
        ...
        nominal_target = Series(np.array(label_name)[numerical_target].ravel()) # Same with nominal categories
